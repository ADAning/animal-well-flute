...
class MusicNotation:
    ...
    RELATIVE_HEIGHTS: Dict[str, float] = {
        ...
        "l1": -6.0, "l2": -5.0, "l3": -4.0, "l4": -3.5, "l5": -2.5, "l6": -1.5, "l7": -0.5,
        
        # 中音区 (相对标记，以1为基准0)
        "1": 0, "2": 1, "3": 2, "4": 2.5, "5": 3.5, "6": 4.5, "7": 5.5,
        
        # 高音区 (相对标记)
        "h1": 6, "h2": 7, "h3": 8, "h4": 8.5, "h5": 9.5, "h6": 10.5, "h7": 11.5,
        
        # 超高音区 (可扩展)
        "h8": 12, "h9": 13, "h10": 14, "h11": 14.5, "h12": 15.5,
        "h13": 16.5, "h14": 17.5, "h15": 18, "h16": 19, "h17": 20,
    }
...
class FlutePhysical:
    ...
    def validate_range(cls, min_height: float, max_height: float) -> bool:
        """验证音域是否在笛子可演奏范围内"""
        return (cls.is_playable(min_height) and 
                cls.is_playable(max_height) and
                (max_height - min_height) <= cls.PHYSICAL_RANGE)
...
class RangeAnalyzer:
    ...
    def analyze_relative_notes(notes: List[RelativeNote]) -> RangeInfo:
        """分析相对音符的音域信息"""
        valid_heights = [note.relative_height for note in notes 
    ...
    def analyze_physical_notes(notes: List[PhysicalNote]) -> RangeInfo:
        """分析物理音符的音域信息"""
        valid_heights = [note.physical_height for note in notes 
