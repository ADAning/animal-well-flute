...
class AutoConverter:
    ...
    def convert_jianpu(
        ...
        Returns:
            物理音符列表
        """
        logger.info(f"Converting jianpu with {len(parsed_jianpu)} bars using {strategy} strategy")
        ...
        if isinstance(strategy, str):
            if strategy == "auto":
                # 自动选择最佳策略，考虑用户偏好
                if auto_preference:
                    physical_notes, used_strategy = self.optimizer.find_best_mapping_with_preference(
                        all_relative_notes, auto_preference
                    )
                else:
                    physical_notes, used_strategy = self.optimizer.find_best_mapping(all_relative_notes)
                logger.info(f"Auto-selected strategy: {used_strategy}")
            elif strategy == "manual":
                if manual_offset is None:
                    raise ValueError("manual_offset is required when using manual strategy")
    ...
    def _log_conversion_summary(self, converted_jianpu: List[List[PhysicalNote]]) -> None:
        """记录转换摘要"""
