...
class AdaptiveMapper:
    ...
    def map_song_to_flute(
        ...
        logger.info(
            f"Song range: {range_info.min_height:.1f} to {range_info.max_height:.1f} "
        ...
        if range_info.span > 24:  # 2个八度
            raise ValueError(
                f"Song range ({range_info.span:.1f} semitones) exceeds 2 octaves limit"
            )

        if range_info.span > self.flute.PHYSICAL_RANGE:
            raise ValueError(
                f"Song range ({range_info.span:.1f} semitones) exceeds flute physical range "
        ...
        logger.info(
            f"Successfully mapped {len(physical_notes)} notes to physical range"
        )
        return physical_notes

    def _calculate_optimal_offset(
        self, range_info: RangeInfo, strategy: MappingStrategy
    ) -> float:
        """计算最优映射偏移量"""
        logger.info(f"Calculating optimal offset for {strategy.value} strategy")
        logger.info(
            f"Range info: {range_info.min_height:.1f} to {range_info.max_height:.1f}"
        )
        logger.info(
            f"Flute range: {self.flute.MIN_PHYSICAL_HEIGHT:.1f} to {self.flute.MAX_PHYSICAL_HEIGHT:.1f}"
        )
        ...
        if strategy == MappingStrategy.OPTIMAL:
            ...
            flute_center = (
                self.flute.MIN_PHYSICAL_HEIGHT + self.flute.MAX_PHYSICAL_HEIGHT
            ) / 2
    ...
    def _map_single_note(
        self, relative_note: RelativeNote, offset: float
    ) -> PhysicalNote:
        """映射单个音符"""
    ...
    def _validate_final_mapping(self, physical_notes: List[PhysicalNote]) -> None:
        ...
        if invalid_notes:
            ...
            for i, note in invalid_notes[:5]:  # 只显示前5个
                error_msg += (
                    f"  Note {i}: {note.notation} -> {note.physical_height:.1f}\n"
                )
        ...
        if not self.flute.validate_range(range_info.min_height, range_info.max_height):
            raise ValueError(
                f"Final mapped range ({range_info.min_height:.1f} to "
    ...
    def get_mapping_suggestions(self, relative_notes: List[RelativeNote]) -> dict:
        ...
        for strategy in [
            ...
            try:
                ...
                suggestions[strategy.value] = {"error": str(e), "feasible": False}
...
class MappingOptimizer:
    ...
    def find_best_mapping_with_preference(
        self, relative_notes: List[RelativeNote], preference: str
    ) -> Tuple[List[PhysicalNote], str]:
        """
        找到最佳映射方案，优先考虑用户偏好
    ...
    def _evaluate_mapping(self, physical_notes: List[PhysicalNote]) -> float:
        ...
        flute_center = (
            self.mapper.flute.MIN_PHYSICAL_HEIGHT
            + self.mapper.flute.MAX_PHYSICAL_HEIGHT
        ) / 2
        centering = 1.0 - abs(center - flute_center) / (
            self.mapper.flute.PHYSICAL_RANGE / 2
        )

        # 3. 边界安全性 (避免接近边界)
        min_margin = (
            range_info.min_height - self.mapper.flute.MIN_PHYSICAL_HEIGHT
        ) / self.mapper.flute.PHYSICAL_RANGE
        max_margin = (
            self.mapper.flute.MAX_PHYSICAL_HEIGHT - range_info.max_height
        ) / self.mapper.flute.PHYSICAL_RANGE
