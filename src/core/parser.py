...
class RelativeParser:
    ...
    def parse_recursively(
        ...
        if not self.is_sequence(elements):
                ...
                if len(elem) == 1:
                    ...
                    if elem == "-":
                        ...
                        if result:
                            ...
                            logger.debug(
                                f"Extended previous note by {note_time_factor}"
                            )
                        return
                else:
            ...
            result.append(note)
            logger.debug(
                f"Parsed relative note: {note_str} "
        ...
        for elem in elements:
            self.parse_recursively(
                elem, result, time_factor=time_factor / 2, major_offset=major_offset
            )
    ...
    def parse(self, jianpu: List[Union[List, float, str]]) -> List[List[RelativeNote]]:
        ...
        for i, bar in enumerate(jianpu):
            ...
            try:
                self.parse_recursively(
                    bar, parsed_bar, time_factor=2.0, major_offset=current_major_offset
                )

                # 扩展音域定义（如果需要）
                max_height = max(
                    (
                        note.relative_height
                        for note in parsed_bar
