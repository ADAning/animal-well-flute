...
class AppConfig:
    ...
    DEFAULT_CONFIG = {
        ...
        # 验证相关
        "validation": {"max_validation_errors_display": 3, "strict_mode": False},
        # 性能相关
        "performance": {
            "enable_caching": True,
        ...
        for env_key, (section, key, value_type) in env_mappings.items():
            ...
            if env_value is not None:
                try:
                    ...
                    logger.debug(
                        f"Override from env {env_key}: {section}.{key} = {converted_value}"
                    )
                except (ValueError, TypeError) as e:
                    logger.warning(
                        f"Invalid environment variable {env_key}={env_value}: {e}"
                    )
    ...
    def supported_image_extensions(self) -> list:
        """支持的图片扩展名"""
        return self.get(
            "image_processing", "supported_extensions", [".png", ".jpg", ".jpeg"]
        )
