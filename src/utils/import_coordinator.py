...
class ImportExecutor:
    ...
    def execute_multi_image(self, image_files: List[Path], provider: str, folder_name: str) -> Dict[str, Any]:
        """执行多张图片合并导入"""
...
class ImportCoordinator:
    ...
    def coordinate_import(self, image_patterns: List[str], ai_provider: Optional[str] = None) -> ImportResult:
        """协调整个导入流程
    ...
    def _execute_grouped_import(self, folder_groups: Dict[Path, List[Path]], provider: str) -> ImportResult:
        """执行分组导入"""
        ...
        for folder_path, files_in_folder in folder_groups.items():
            ...
            try:
                if len(files_in_folder) == 1:
                    # 单张图片
                    print(f"\n📄 处理单张图片: {folder_name}")
                    import_result = self.executor.execute_single_image(files_in_folder[0], provider)
                else:
                    # 多张图片合并
                    print(f"\n🎵 合并文件夹 '{folder_name}' 中的 {len(files_in_folder)} 张图片...")
                    import_result = self.executor.execute_multi_image(files_in_folder, provider, folder_name)
