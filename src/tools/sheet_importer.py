...
class SheetPreprocessor:
    ...
    def preprocess_image(self, image_path: Path) -> Tuple[bytes, str]:
        ...
        try:
            with Image.open(image_path) as img:
                ...
                img.save(
                    img_bytes, format="JPEG", quality=self.jpeg_quality, optimize=True
                )
    ...
    def _split_horizontally(self, img: Image.Image, original_path: Path) -> List[Path]:
        ...
        for i in range(num_parts):
            ...
            part_path = (
                temp_dir / f"{original_path.stem}_part_{i+1}{original_path.suffix}"
            )
...
class JianpuSheetImporter:
    ...
    def __init__(
        self, config: Optional[ToolsConfig] = None, songs_dir: Optional[Path] = None
    ):
        """初始化导入器
    ...
    def _import_single_part(
        ...
        recognition_result = self.recognizer.recognize(
            image_data, image_format, provider
        )
    ...
    def _generate_yaml_file(
        self, recognition_result: Dict[str, Any], output_name: str
    ) -> Dict[str, Any]:
        """生成YAML文件
        ...
        try:
            ...
            while output_path.exists():
                output_path = (
                    original_path.parent / f"{original_path.stem}_{counter}.yaml"
                )
                counter += 1
            ...
            if has_validation_errors:
                ...
                result["warning_message"] = (
                    f"文件已生成但存在验证警告: {validation_errors}"
                )
                # 添加AI响应信息用于调试
            ...
            return {"success": False, "error": f"YAML generation failed: {e}"}

    def import_batch(
        self, image_paths: List[Path], provider: Optional[str] = None
    ) -> Dict[str, Any]:
        """批量导入简谱图片
        ...
        for i, image_path in enumerate(image_paths):
            ...
            results.append({"image_path": str(image_path), "result": result})
    ...
    def import_multiple_images(
        ...
        try:
            ...
            for i, image_path in enumerate(image_paths):
                ...
                # 预处理图片
                image_data, image_format = self.preprocessor.preprocess_image(
                    image_path
                )
            ...
            # 生成YAML文件
            output_result = self._generate_yaml_file(
                combined_result, output_name or "multi_image_song"
            )
            ...
            return {"success": False, "error": f"Multi-image import failed: {e}"}

    def import_folder_as_single_song(
        self,
        folder_path: Path,
        provider: Optional[str] = None,
        output_name: Optional[str] = None,
        ...
        result = self.import_multiple_images(
            image_files, provider=provider, output_name=output_name or folder_path.name
        )

        return result

    def _combine_multi_image_results(
        self,
        ...
        for result in recognition_results:
            ...
            if (
                title
                and title.strip()
                and title.lower() not in ["null", "none", "未知"]
            ):
        ...
        # 添加合并信息
        notes.append(
            f"由{len(recognition_results)}张图片合并而成，共{len(all_sections)}行简谱。"
        )
        ...
        return {
            ...
            "success": True,
            "provider": (
                recognition_results[0].get("provider", "unknown")
